...
void checkShaderCompilation(unsigned int objectID) {
	int s;
	char infoLog[512];
	glGetShaderiv(objectID, GL_COMPILE_STATUS, &s);
...
void checkProgramCompilation(unsigned int objectID) {
	int s;
	char infoLog[512];
	glGetProgramiv(objectID, GL_LINK_STATUS, &s);
	if (!s) {
		glGetProgramInfoLog(objectID, 512, NULL, infoLog);
		std::cout << "program compilation error: " << infoLog << std::endl;
	}
	else {
		std::cout << "program compiled ok" << std::endl;
	}
}
...
int main(int args, char* argv[]) {
	...
	//making and compiling VERTEX SHADER object
	...
	//shader program object so that linking v and f shader are possible
	unsigned int shaderProgram;
	shaderProgram = glCreateProgram();
	glAttachShader(shaderProgram, vertexShader);
	glAttachShader(shaderProgram, fragmentShader);
	glLinkProgram(shaderProgram);
	checkProgramCompilation(shaderProgram);//only for shaders

	glUseProgram(shaderProgram);

	//free gpu memory after linking
